...
1. **PDF Parsing**: Uses `pdfplumber` to automatically detect and extract all tables from the PDF
...
### v1.1 (April 2026)
- Coordinate-based fallback for borderless statements (e.g. Zapphaire-style)
- Period labels and bank header text aligned with PDF where applicable
...
- Rebuild with `python build.py`; executable is `GAC-PDF-EXCEL-CONVERTER.exe` in this folder
